...
# ===================== CSV REPORT =====================
def generate_csv_report(art_dir: str, name: str, logs: list):
    csv_path = os.path.join(art_dir, "cursor_report.csv")
    with open(csv_path, "w", newline="", encoding="utf-8-sig") as f:
        writer = csv.writer(f)
        writer.writerow(["cursor", "position", "item_id", "is_pinned", "intra_dup", "cross_dup_first_seen_at"])
        for entry in logs:
            cursor = entry["cursor"]
            pinned_set = set(entry.get("pinned_ids", []))
            check_ids = entry.get("ids", [])
            intra_dups = set(entry.get("tc01_intra_dup_sample", []))
            cross_dups = {x["id"]: x["first_cursor"] for x in entry.get("dup_with_previous", [])}

            for pos, _id in enumerate(check_ids, 1):
                writer.writerow([
                    cursor,
                    pos,
                    _id,
                    "YES" if _id in pinned_set else "NO",
                    "DUP" if _id in intra_dups else "",
                    cross_dups[_id] if _id in cross_dups else "",
                ])
...
def run_check(placement: dict, max_cursors: int = 10) -> dict:
    ...
    # ✅ Generate CSV report
    generate_csv_report(art_dir, name, logs)
