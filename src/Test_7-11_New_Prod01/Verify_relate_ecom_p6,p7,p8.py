...
REPORT_DIR = os.path.join(SCRIPT_DIR, "reports", "DMPREC-9588")
...
PLACEMENTS = [
    {
        "name": "sfv-p6",
        "prefer_last_meta": True,   # metadata returns 2 rows per id; use the last (2nd) one
        "url": (
    ...
    {
        "name": "sfv-p7",
        "prefer_last_meta": False,  # metadata returns 2 rows per id; use the first (1st) one
        "url": (
            "http://ai-universal-service-711.prod-gcp-ai-bn.ai-platform.gcp.dmp.true.th"
...
# Helpers
# =============================================================
def fetch_metadata_batch(ids: list[str], fields: list[str], prefer_last: bool = False) -> dict:
    if not ids:
        return {}
    payload = {
        "parameters": {"id": ids, "fields": fields},
        "options": {"cache": False},
    }
    ...
    for item in items:
        if "id" in item:
            ...
            if item_id not in result:
                ...
                result[item_id] = item  # always overwrite → keep last occurrence
    return result


def fetch_metadata_all(ids: list[str], fields: list[str], prefer_last: bool = False) -> dict:
    result = {}
    for i in range(0, len(ids), METADATA_BATCH_SIZE):
        ...
        result.update(fetch_metadata_batch(batch, fields, prefer_last=prefer_last))
    return result
...
def run_check(placement: dict) -> dict:
    name = placement["name"]
    url = placement["url"]
    prefer_last_meta = placement.get("prefer_last_meta", False)
    ...
    for fid in final_ids:
        ...
        for rid in relate_ids:
            ...
            if content_type == "ecommerce":
                ...
                values = partner_related if isinstance(partner_related, list) else [partner_related]
                if EXPECTED_PARTNER in values:
    ...
    assert accounted == len(final_ids), (
        f"[{name}] Count mismatch: final_ids={len(final_ids)}, "
        f"pass={len(pass_items)}, "
        f"fail_no_relate={len(fail_no_relate)}, "
        f"fail_no_ecommerce={len(fail_no_ecommerce)}, "
        f"fail_no_partner_related={len(fail_no_partner_related)}"
    )
...
# =============================================================
def _assert_summary(report: dict):
    summary = report["summary"]
    fail_msgs = []

    if summary["fail_empty_relate_count"] > 0:
        ids = [x["id"] for x in report["fail_no_relate"]]
        fail_msgs.append(
            f"FAIL empty relate_content ({summary['fail_empty_relate_count']} items): {ids[:10]}"
        )

    if summary["fail_no_ecommerce_count"] > 0:
        ids = [x["id"] for x in report["fail_no_ecommerce"]]
        fail_msgs.append(
            f"FAIL no ecommerce ({summary['fail_no_ecommerce_count']} items): {ids[:10]}"
        )

    if summary["fail_no_partner_related_count"] > 0:
        ids = [x["id"] for x in report["fail_no_partner_related"]]
        fail_msgs.append(
            f"FAIL ecommerce missing partner_related={PARTNER_ID} "
            f"({summary['fail_no_partner_related_count']} items): {ids[:10]}"
        )

    assert not fail_msgs, "\n".join(fail_msgs)


def test_verify_relate_ecom_sfv_p6():
    report = run_check(PLACEMENTS[0])
    _assert_summary(report)


def test_verify_relate_ecom_sfv_p7():
    report = run_check(PLACEMENTS[1])
    _assert_summary(report)


def test_verify_relate_ecom_sfv_p8():
    report = run_check(PLACEMENTS[2])
    _assert_summary(report)
