"""
test_gameitem_parameters.py

Test script สำหรับตรวจสอบ parameters ของ Metadata API (game_item)
แต่ละ test จะ:
...
Test Cases — STANDARD:
  ...
  5.  id filter      — IDs ที่ออกมาอยู่ใน id list ที่กำหนดเท่านั้น
  6.  filter_out_category — ทุก items ไม่มี article_category ที่ถูก filter ออก
...
# CONFIG
# ======================================================
API_URL = "http://ai-metadata-service.preprod-gcp-ai-bn.int-ai-platform.gcp.dmp.true.th/metadata/game_item"
...
ID_LIST = [
    "03qAkQdlmdo3","042E7a3wovD9","05QG07meW2kn","06pXbodR8QO6","08d0D1AEr1o8",
    "0BnoePG95pxO","0D78M96VzmJO","0G0xZ5P8ZBeG","0K5nV9Yv8BND","0VmaeP4JN7KV",
    "1BbZwy71PEAB"
]

# category ที่ถูก filter ออก
FILTER_OUT_CATEGORY = ["cloud"]

# ID ที่ใช้ทดสอบ exclude_ids
EXCLUDE_ID = "0G0xZ5P8ZBeG"
...
def build_existence_sql(ids: list) -> str:
    """SQL ง่ายๆ เช็คว่า IDs จาก API มีใน mst_gameitem_nonprod ไหม"""
...
def test_tophit_date_filter_50():
    """filter=50 ควรได้ items >= filter=30 (window กว้างกว่า) และ IDs match Spanner"""
    resp30  = call_api({"limit": 100, "language": "th", "tophit_date_filter": 30})
    resp50  = call_api({"limit": 100, "language": "th", "tophit_date_filter": 50})
    src30   = get_sources(resp30)
    src50   = get_sources(resp50)
    count30 = len(src30)
    count50 = len(src50)
    print(f"    filter=30 → {count30} items")
    print(f"    filter=50 → {count50} items")

    if count50 < count30:
        return False, (
            f"filter=50 ({count50}) น้อยกว่า filter=30 ({count30}) — ผิดปกติ"
        )

    api_ids = get_ids(src50)
    sp_ok, sp_msg, _ = compare_with_spanner("tophit_date_filter=50", api_ids)
    if not sp_ok:
        return False, f"date_filter OK แต่ {sp_msg}"
    return True, f"filter=30→{count30} items, filter=50→{count50} items และ {sp_msg}"


# ── 5. id filter ────────────────────────────────────────
def test_id_filter():
    """IDs ที่ออกมาต้องอยู่ใน ID_LIST เท่านั้น และ match Spanner"""
    resp    = call_api({"limit": 100, "language": "th", "id": ID_LIST})
    sources = get_sources(resp)
    if not sources:
...
def test_filter_out_category():
    ...
    resp    = call_api({"limit": 100, "language": "th",
                        "filter_out_category": FILTER_OUT_CATEGORY})
    sources = get_sources(resp)
    if not sources:
        return False, "ไม่มี items ใน response"
...
def test_exclude_ids_when_set():
    ...
    resp    = call_api({"limit": 200, "language": "th",
                        "id": ID_LIST, "exclude_ids": [EXCLUDE_ID]})
    sources = get_sources(resp)
    returned_ids = [s.get("id") for s in sources]
...
def test_exclude_ids_when_not_set():
    ...
    resp    = call_api({"limit": 200, "language": "th", "id": ID_LIST})
    sources = get_sources(resp)
    returned_ids = [s.get("id") for s in sources]
...
if __name__ == "__main__":
    print("=" * 60)
    print("🚀  Game Item — Parameter Test Suite (Standard + Latest)")
    print(f"    API     : {API_URL}")
    ...
    run_test("5.  id filter — IDs อยู่ใน id list เท่านั้น",             test_id_filter)
    run_test("6.  filter_out_category — ไม่มี category ต้องห้าม",       test_filter_out_category)
    ...
    report_path = os.path.join(out_dir, "test_gameitem_result.json")
