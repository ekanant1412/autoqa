"""
test_sfvseries_parameters.py

Test script สำหรับตรวจสอบ parameters ของ Metadata API (sfv_series)
แต่ละ test จะ:
...
Test Cases — STANDARD:
  ...
  6.  id filter      — IDs ที่ออกมาอยู่ใน id list ที่กำหนดเท่านั้น
  7.  fields 4       — _source มีครบ 4 fields ตามที่กำหนด
  8.  filter_out_category — ทุก items ไม่มี article_category ที่ถูก filter ออก
  9.  exclude_ids (excluded) — ID ที่กำหนดไม่ปรากฏใน response
  10. exclude_ids (included) — ID นั้นปรากฏใน response เมื่อไม่กำหนด exclude_ids
...
ID_LIST = [
    "1B21Y1qGq57d","4AO8w84l31mL","6XzMQMM1QN1k","XqvPXJkMEovn"
]

# 4 fields จาก REQUEST_BODY_STANDARD
FIELDS_4 = ["id", "create_date", "title", "article_category"]

# category ที่ถูก filter ออก
FILTER_OUT_CATEGORY = ["series"]

# ID ที่ใช้ทดสอบ exclude_ids
EXCLUDE_ID = "4AO8w84l31mL"
...
def build_existence_sql(ids: list) -> str:
    """SQL ง่ายๆ เช็คว่า IDs จาก API มีใน mst_sfvseries_nonprod ไหม"""
...
def test_title_contains_rak():
    ...
    resp    = call_api({"limit": 50, "language": "th", "title": "รัก", "fields": FIELDS_4})
    sources = get_sources(resp)
    if not sources:
...
# ── 7. fields — ครบ 4 fields ตามที่กำหนด ───────────────
def test_fields_5():
    """กำหนด fields=FIELDS_4 → _source ทุก item ต้องมีครบ 4 fields พอดี และ IDs match Spanner"""
    ...
    print(f"    fields → ทุก {len(sources)} items มีครบ 4 fields")
    api_ids = get_ids(sources)
    sp_ok, sp_msg, _ = compare_with_spanner("fields-5", api_ids)
    if not sp_ok:
        return False, f"fields OK แต่ {sp_msg}"
    return True, f"ครบ 4 fields: {FIELDS_4} และ {sp_msg}"


# ── 8. filter_out_category ─────────────────────────────
def test_filter_out_category():
    """ทุก items ต้องไม่มี article_category ใน FILTER_OUT_CATEGORY และ match Spanner"""
    resp    = call_api({"limit": 100, "language": "th",
                        "filter_out_category": FILTER_OUT_CATEGORY, "fields": FIELDS_4})
    sources = get_sources(resp)
    if not sources:
        return False, "ไม่มี items ใน response"
...
# ── 9. exclude_ids — ID ไม่ปรากฏเมื่อกำหนด ────────────
def test_exclude_ids_when_set():
    ...
    resp    = call_api({"limit": 200, "language": "th",
                        "id": ID_LIST, "exclude_ids": [EXCLUDE_ID], "fields": FIELDS_4})
    sources = get_sources(resp)
    returned_ids = [s.get("id") for s in sources]
...
# ── 10. exclude_ids — ID ปรากฏเมื่อไม่กำหนด ────────────
...
if __name__ == "__main__":
    print("=" * 60)
    print("🚀  SFV Series — Parameter Test Suite (Standard + Tophit + Latest)")
    print(f"    API     : {API_URL}")
    ...
    run_test("6.  id filter — IDs อยู่ใน id list เท่านั้น",             test_id_filter)
    run_test("7.  fields — ครบ 4 fields ตามที่กำหนด",                   test_fields_5)
    run_test("8.  filter_out_category — ไม่มี category ต้องห้าม",       test_filter_out_category)
    run_test("9.  exclude_ids — ID ไม่ปรากฏเมื่อกำหนด",                 test_exclude_ids_when_set)
    run_test("10. exclude_ids — ID ปรากฏเมื่อไม่กำหนด",                 test_exclude_ids_when_not_set)
    ...
    report_path = os.path.join(out_dir, "test_sfvseries_result.json")
