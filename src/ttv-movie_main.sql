...
BaseDataRaw AS (
    ...
    AND (ARRAY_LENGTH((SELECT p_filter_out_category FROM Params)) = 0
         OR NOT EXISTS(SELECT 1 FROM UNNEST(m.article_category) AS x WHERE x IN UNNEST((SELECT p_filter_out_category FROM Params))))
    ...
    AND (m.publish_date <= TIMESTAMP_TRUNC(CURRENT_TIMESTAMP(), HOUR))
    AND m.content_provider NOT IN ('true_vision')
    AND (m.expire_date > TIMESTAMP_TRUNC(CURRENT_TIMESTAMP(), HOUR))
