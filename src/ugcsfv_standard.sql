...
BaseDataRaw AS (
		SELECT
			m.id, m.thumb, m.title, m.article_category, m.content_type, m.create_date, m.publish_date, m.tags, m.source_country, m.lang, m.relate_content, m.create_by, m.update_by, m.related_ecommerce_id, m.ugc_flag,
			0 AS hit_7,
			0.0 AS search_score,
...
MainResults AS (
		SELECT
			id, thumb, title, article_category, content_type, create_date, publish_date, tags, source_country, lang, relate_content, create_by, update_by, related_ecommerce_id, ugc_flag,
