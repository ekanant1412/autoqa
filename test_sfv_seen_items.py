...
def extract_reserved_positions_ids(node: Optional[dict]) -> set:
    """
    ดึง id จาก node.result.reservedPositions
    format: "{pool_name}_{id}" → กรอง pin_*/live_* ออก → ตัดเหลือแค่ id
    """
    if not node:
        return set()
...
def extract_items_ids_limit(node: Optional[dict], limit: int = 5) -> set:
    """ดึง id จาก node.result.items[].id เฉพาะ limit รายการแรก"""
    if not node:
        return set()
...
def _build_dag_cache(
    ...
    """Fetch ทุก cursor สำหรับ DAG หนึ่ง คืน {cursor_num: data}"""
    ...
    for cursor in range(1, max_cursors + 1):
        try:
            ...
            if status == 404:
                # 404 = ไม่มีข้อมูลสำหรับ cursor นี้ → ถือว่าหมด cursor แล้ว
...
def evidence_dir(pytestconfig):
    sso_id = pytestconfig.getoption("--ssoId") or "unknown"
...
def cursor_cache(pytestconfig, evidence_dir):
    """
    Fetch ทุก cursor ของทุก DAG ที่เลือก (ทำครั้งเดียวต่อ session)
    คืนค่า dict[dag_name → dict[cursor_num → data]]
    """
    sso_id = pytestconfig.getoption("--ssoId")
    if not sso_id:
        pytest.fail("ต้องระบุ --ssoId เช่น: pytest test_sfv_seen_items.py --ssoId 1468")

    dag_option    = pytestconfig.getoption("--dag")
    selected_dags = _parse_dag_option(dag_option)
    max_cursors   = int(pytestconfig.getoption("--max-cursors"))
...
def pytest_generate_tests(metafunc):
    # ── dag_name: parametrize ตาม --dag option ──
    if "dag_name" in metafunc.fixturenames:
        dag_option    = metafunc.config.getoption("--dag", default="all")
        selected_dags = _parse_dag_option(dag_option)
        metafunc.parametrize("dag_name", selected_dags, ids=selected_dags)

    # ── cursor_num: parametrize ตาม scope ของแต่ละ test ──
    if "cursor_num" in metafunc.fixturenames:
        max_cursors = int(metafunc.config.getoption("--max-cursors", default="5"))
...
class TestSfvSeenItems:
    """
    ตรวจสอบ seen-item consistency ของ sfv-p4 / sfv-p5 / sfd-p1 ข้าม cursor

    รันด้วย:
        pytest test_sfv_seen_items.py --ssoId <ID> --dag all -v

    Evidence จะถูกเซฟที่:
        reports/sfv_seen_items/sso<ID>_<timestamp>/<dag>/
    """

    # ── Original checks (check1-6) ────────────────────────────────────────────
    ...
    def test_check1_set_seen_equals_combined(self, dag_name, cursor_num, cursor_cache):
        ...
        Skip: เมื่อ get_final_result_redis มี item
        """
        ...
        if data["skip_check1"]:
            pytest.skip(
                f"[{dag_name}] cursor {cursor_num}: get_final_result_redis มี item "
                f"({len(data['final_result_ids'])} items) — Check 1 ไม่ applicable"
            )

        combined = data["current_combined"]
        set_seen = data["set_seen_ids"]

        assert combined == set_seen, diff_msg(
            combined, set_seen,
            f"[{dag_name}] logic_filter∪insert_relevance (cursor {cursor_num})",
            "set_seen_items_redis",
        )
    ...
    def test_check2_get_seen_equals_accumulated(self, dag_name, cursor_num, cursor_cache):
        """
        [Check 2] cursor >= 2:
        get_seen_item_redis.result.items[].id == set_seen_items_redis.ids ของ cursor ก่อนหน้า
        """
        if cursor_num < 2:
            pytest.skip("Check 2 เริ่มตั้งแต่ cursor 2 เป็นต้นไป")

        data = cursor_cache.get(dag_name, {}).get(cursor_num)
        if data is None:
            pytest.skip(f"[{dag_name}] cursor {cursor_num} ไม่ถูก fetch (items หมดก่อน)")

        prev_set_seen = data["prev_set_seen_ids"]
        get_seen      = data["get_seen_ids"]

        assert prev_set_seen == get_seen, diff_msg(
            prev_set_seen, get_seen,
    ...
    def test_check3_merge_seen_equals_set_seen(self, dag_name, cursor_num, cursor_cache):
        """
        [Check 3] cursor >= 2:
        (merge_seen ∪ logic_filter[:5] ∪ insert_relevance) == set_seen_items_redis.ids
        """
        if cursor_num < 2:
            pytest.skip("Check 3 เริ่มตั้งแต่ cursor 2 เป็นต้นไป")

        data = cursor_cache.get(dag_name, {}).get(cursor_num)
        if data is None:
            pytest.skip(f"[{dag_name}] cursor {cursor_num} ไม่ถูก fetch (items หมดก่อน)")

        check3_combined = data["check3_combined"]
        set_seen        = data["set_seen_ids"]

        assert check3_combined == set_seen, diff_msg(
            check3_combined, set_seen,
    ...
    def test_check4_get_seen_empty_on_cursor1(self, dag_name, cursor_num, cursor_cache):
        """
        [Check 4] cursor 1:
        get_seen_item_redis ต้องว่างเปล่า (ยังไม่มี seen ก่อนหน้า)
        """
        data = cursor_cache.get(dag_name, {}).get(cursor_num)
        if data is None:
        ...
        assert get_seen == set(), (
            ...
            f"แต่พบ {len(get_seen)} items: {sorted(get_seen)[:10]}"
        )

    def test_check5_merge_seen_composition(self, dag_name, cursor_num, cursor_cache):
        """
        [Check 5] cursor >= 2:
        5a) get_seen ⊆ merge_seen
        5b) (merge_seen − get_seen) ⊆ get_final_result_redis[prev]
        """
        if cursor_num < 2:
            pytest.skip("Check 5 เริ่มตั้งแต่ cursor 2 เป็นต้นไป")
        ...
        assert c5a["passed"], (
            f"[{dag_name}] cursor {cursor_num} [5a]: get_seen มี {c5a['count_expected']} items "
            f"แต่พบแค่ {c5a['count_actual']} ใน merge_seen — ขาด: {c5a['missing_in_actual'][:10]}"
        )
        assert c5b["passed"], (
            f"[{dag_name}] cursor {cursor_num} [5b]: merge_seen มี items ที่ไม่ได้มาจาก seen หรือ "
            f"final_result: {c5b['extra_in_actual'][:10]}"
        )

    def test_check6_merge_seen_not_in_logic_filter(self, dag_name, cursor_num, cursor_cache):
        """
        [Check 6] cursor >= 2:
        merge_seen ∩ logic_filter_overlap_items_pin_and_live == ∅
        (seen items ต้องไม่โผล่ใน logic_filter — filter ทำงานถูกต้อง)
        """
        if cursor_num < 2:
            pytest.skip("Check 6 เริ่มตั้งแต่ cursor 2 เป็นต้นไป")

        data = cursor_cache.get(dag_name, {}).get(cursor_num)
        if data is None:
            pytest.skip(f"[{dag_name}] cursor {cursor_num} ไม่ถูก fetch (items หมดก่อน)")

        overlap = data["overlap6"]
        assert not overlap, (
            f"[{dag_name}] cursor {cursor_num}: พบ {len(overlap)} seen items ใน logic_filter: "
            f"{sorted(overlap)[:20]}"
        )

    # ── New explicit test cases ────────────────────────────────────────────────

    def test_set_seen_redis_special_and_first5(self, dag_name, cursor_num, cursor_cache):
        """
        Verify set_seen_redis node with logic keeping special items and the first 5 items
        except pin and live items from latest node.
        ...
        Runs:   cursor 1 – max_cursors  ×  ทุก DAG
        Skip:   เมื่อ get_final_result_redis มี item
        """
        data = cursor_cache.get(dag_name, {}).get(cursor_num)
        if data is None:
        ...
        if data["skip_check1"]:
            pytest.skip(
                f"[{dag_name}] cursor {cursor_num}: get_final_result_redis มี item "
                f"({len(data['final_result_ids'])} items) — set_seen รวม merged-seen แล้ว"
            )
    ...
    def test_set_seen_redis_prev_equals_get_seen(self, dag_name, cursor_num, cursor_cache):
        """
        Verify set_seen_redis node previous cursor == get seen redis.

        Assertion:
            get_seen_item_redis.result.items[].id  (cursor N)
              == set_seen_items_redis.result.ids    (cursor N-1)

        Runs:   cursor 2 – max_cursors  ×  ทุก DAG
        """
        data = cursor_cache.get(dag_name, {}).get(cursor_num)
        if data is None:
            pytest.skip(f"[{dag_name}] cursor {cursor_num} ไม่ถูก fetch (items หมดก่อน)")
    ...
    def test_merge_seen_all_items_from_prev_cursor(self, dag_name, cursor_num, cursor_cache):
        """
        Verify merge_seen node keeping all items
        (final_result + first 5 items + special items) from previous cursor.

        Assertion:
            ( merge_seen_and_prev_page_items.result.items[].id       ← final_result (prev)
            ∪ logic_filter_overlap_items_pin_and_live.result.items[:5] ← first 5 items
            ∪ insert_relevance_creator_candidates.result.reservedPositions ← special items
            )  ==  set_seen_items_redis.result.ids
