...
# ── sfv seen-items CLI options (default อ่านจาก env vars ถ้าไม่ได้ส่งทาง CLI) ──
def pytest_addoption(parser):
    try:
        parser.addoption(
            "--ssoId",
            action="store",
            default=os.environ.get("SFV_SSO_ID"),
            help="SSO ID สำหรับ sfv seen-item tests",
        )
        parser.addoption(
            "--dag",
            action="store",
            default=os.environ.get("SFV_DAG", "all"),
            help="DAG ที่จะทดสอบ: all | sfv-p4 | sfv-p5 | sfd-p1",
        )
        parser.addoption(
            "--max-cursors",
            action="store",
            default=os.environ.get("SFV_MAX_CURSORS", "5"),
            help="จำนวน cursor สูงสุดที่จะทดสอบ",
        )
        parser.addoption(
            "--save-responses",
            action="store_true",
            default=False,
            help="บันทึก raw JSON response ทุก cursor",
        )
    except ValueError:
        pass  # options ถูก register แล้วโดย conftest อื่น
