...
WITH
  ...
  SELECT
    ...
    CAST([] AS ARRAY<STRING>) AS p_merchant_type
  ),
-- ============================================================
-- BASE DATA + FILTERS
-- ============================================================
BaseDataRaw AS (
		SELECT
			m.id, m.original_id, m.location, m.address, m.thumb_list, m.privilege_list, m.discover, m.merchant_type, m.title, m.content_type, m.publish_date, m.detail, m.tags, m.article_category, m.setting, m.most_popular,
			0 AS hit_7,
			0.0 AS search_score,
     ...
     LIMIT 1
      ...
      0.0
		  ...
		  FROM mst_trueyoumerchant_nonprod m
		WHERE
		    m.status = 'publish'
    ...
    AND (ARRAY_LENGTH((SELECT p_merchant_type FROM Params)) = 0 OR m.merchant_type IN UNNEST((SELECT p_merchant_type FROM Params)))
    AND (m.publish_date <= CURRENT_TIMESTAMP())
...
-- ============================================================
SELECT
			id, original_id, location, address, thumb_list, privilege_list, discover, merchant_type, title, content_type, publish_date, detail, tags, article_category, setting, most_popular,
