...
WITH
  ...
  SELECT
    ...
    ['s-highlight-epl,epl-poll-whowin,epl-preview,epl-h2h,epl-key-player'] AS p_filter_out_category,
    ...
    0 AS p_category_date_filter_day,
    30 AS p_tophit_date_filter,
    '' AS p_sort_field,
    '' AS p_sort_field_value,
    CAST(NULL AS FLOAT64) AS p_last_score,
    CAST(NULL AS TIMESTAMP) AS p_last_publish_date,
    '' AS p_last_id,
    50 AS p_agg_tophit_limit,
    'PLAY_COUNT_DAY_14' AS p_agg_tophit_sort_by
  ),
-- ============================================================
...
BaseDataRaw AS (
		SELECT
			...
			COALESCE(m.hit_count_day_7, 0) AS hit_7,
			CAST(JSON_VALUE(m.most_popular, '$.PLAY_COUNT_DAY_14') AS INT64) AS agg_tophit_sort_col,
			0.0 AS search_score,
      ...
      WHEN ARRAY_LENGTH((SELECT p_more_likes FROM Params)) > 0 THEN
        CAST(
          ...
          + CASE WHEN 'article_category' IN UNNEST((SELECT p_similar_fields FROM Params)) AND ARRAY_LENGTH(ref.reference_categories) > 0
                 ...
                 ELSE 0 END
        AS FLOAT64)
      ELSE COALESCE(CAST(m.hit_count_day_7 AS FLOAT64), 0.0)
...
BaseData AS (
			...
			-- ============================================================
-- TOP-HIT PER GROUP (ARRAY_AGG replaces ROW_NUMBER PARTITION BY)
-- ============================================================
TopHitAggGrouped AS (
			SELECT
				agg_group_key,
				ARRAY_AGG(id ORDER BY agg_tophit_sort_col DESC, id ASC LIMIT 50) AS ids
			FROM BaseData
			WHERE agg_tophit_sort_col IS NOT NULL
  AND publish_date >= TIMESTAMP_SUB(CURRENT_TIMESTAMP(), INTERVAL (SELECT p_tophit_date_filter FROM Params) DAY)
			...
			SELECT
				...
				'TOPHIT_AGG' AS result_type,
				CAST(2 AS INT64) AS agg_rank
			FROM TopHitAggGrouped lag
