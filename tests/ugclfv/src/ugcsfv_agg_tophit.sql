...
WITH
  ...
  SELECT
    ...
    'PLAY_COUNT_DAY_30' AS p_agg_tophit_sort_by
  ),
-- ============================================================
...
BaseDataRaw AS (
		SELECT
			...
			0 AS hit_7,
			CAST(JSON_VALUE(m.most_popular, '$.PLAY_COUNT_DAY_30') AS INT64) AS agg_tophit_sort_col,
			0.0 AS search_score,
...
TopHitAggGrouped AS (
			...
			FROM BaseData
			WHERE agg_tophit_sort_col IS NOT NULL AND agg_tophit_sort_col > 0
